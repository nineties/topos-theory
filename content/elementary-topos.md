...
引き戻しの図式の一方を定数関数 $\mathrm{true}: 1\rightarrow \\{\mathrm{true}, \mathrm{false}\\}$ 、もう一方を特性関数とすれば冒頭で説明した部分集合の構成が出来る。すなわち、以下において $Y$ が引き戻しならば($X\times 1\cong X$ であるから)
...
$$ \mathcal{C}(x, \Omega) \cong \mathrm{Sub}(x) $$
...
$$ \Omega^{a+b}\cong \Omega^a \times \Omega^b$$

といった同型が存在する。この右辺はカルテシアン閉圏では任意の$a,b$ に対して必ず存在するので、逆に

$$ \Omega^x \cong \Omega^a \times \Omega ^b $$
...
{{% proposition %}}
点での評価関手は極限を保つ。
$$ \mathrm{ev}\_a\left(\varprojlim F\right)\cong \varprojlim \mathrm{ev}\_a\circ F $$
{{% /proposition %}}
...
$\hat{\mathcal{C}}(x\times p, q) \cong\hat{\mathcal{C}}(x, q^p)$
...
$$\begin{aligned}
\hat{\mathcal{C}}(x, q^p) &\cong \hat{\mathcal{C}}\left(\varinjlim\_{i\in\mathcal{J}}\mathcal{Y}(d_i), q^p\right)\cong\varprojlim\_{i\in\mathcal{J}} \hat{\mathcal{C}}(\mathcal{Y}(d_i), q^p)
\cong \varprojlim\_{i\in\mathcal{J}} q^p(d_i) = \varprojlim\_{i\in\mathcal{J}}\hat{\mathcal{C}}(\mathcal{Y}(d_i)\times p, q) \\\\
&\cong \hat{\mathcal{C}}\left(\varinjlim\_{i\in\mathcal{J}}\mathcal{Y}(d_i)\times p, q\right) = \hat{\mathcal{C}}(x\times p, q)
\end{aligned}$$
$\square$
{{% /details %}}
...
$$\begin{aligned}
A(c) &\cong \\{x\in U(c) \mid \chi_c(x) = \max_{\leq}(\mathrm{Sub}(\mathcal{Y}(c))\\} \\\\
     & = \\{x \in U(c) \mid 1_c \in \chi_c(x)\\}
\end{aligned}$$
すなわち $A(c)\cong \mathrm{Im}(m_c)$ であるので
...
$a,b$ の積・余積をそれぞれ $a\wedge b, a\vee b$ と書く。束であり、始対象と終対象を持つものを **有界束(bounded lattice)** という。始対象(最小元)を $\bot$、終対象(最大元)を $\top$ と書く。
...
指数対象 $b^a$ を $a\Rightarrow b$ と書く。指数対象 $a\rightarrow b$ の事を **含意(implication)** とも呼ぶ。

また、$a\Rightarrow\bot$ を $a$ の **擬似補(pseudo complement)** といい $\neg a$ と書く。
{{% /definition %}}

指数対象とは、$b,c$ について自然な同型
$ \mathcal{H}(c\wedge a, b) \cong \mathcal{H}(c, a\Rightarrow b) $
...
{{% example %}}
位相空間 $X$ の開集合系 $\mathcal{O}\_X$ を包含関係によって半順序集合と見なしたものはハイティング代数となる。また、含意は
$$ (U\Rightarrow V) = \bigcup\\{ W\in\mathcal{O}_X \mid U\cap W\subseteq V \\}$$
にて定まる。 また
$$ \neg U = (U\Rightarrow\emptyset) = \bigcup\\{ W\in\mathcal{O}(x) \mid U\cap W = \emptyset \\}$$
である。
{{% /example %}}
{{% details 証明 %}}
$\mathcal{O}\_X$ が有界束である事はすぐわかるから、$U\Rightarrow V$ が指数対象である事を示す。その為には、任意の開集合 $V$ と $A\cap U\subseteq V$ を満たす開集合 $A$ に対して、以下が可換である事を示せば良い。$A\xhookrightarrow{} (U\Rightarrow V)$ の一意性は明らか。

$$\xymatrix{
A\cap U \ar[d] \ar[rd] & \\\\
(U\Rightarrow V)\cap U \ar[r] & V
}$$

定義より $A\cap U\subseteq (U\Rightarrow V)$ は明らか。また $A\cap U\subseteq U$ より $A\cap U \subseteq (U\rightarrow V)\cap U$ である。また、定義より $(U\Rightarrow V)\subseteq V$ であることも分かるので $(U\Rightarrow V)\cap U\subseteq V$ $\square$
{{% /details %}}

上で述べた、ド・モルガンの法則を確認してみよう。
例えば $\mathbb{R}$ 上の開集合として $U=(-\infty,0), V=(0,\infty)$ とすると
$ \neg U = (0, \infty), \neg V= (-\infty, 0)$なので
$$ \neg U \cap \neg V = \emptyset $$
...
{{% definition title="フレーム" label="def.frame" %}}
完備ハイティング代数 $\mathcal{H}$、すなわち、任意の部分集合 $S\subseteq\mathcal{H}$ に対して上限 $\bigvee S$ と下限 $\bigwedge S$ が存在するものを、
**フレーム(frame)** という。
{{% /definition %}}
位相空間 $X$ の開集合系 $\mathcal{O}\_X$ はフレームである。無限個の開集合の共通部分は開集合とは限らないからその内部をとる。
$$ \bigvee U\_{\lambda} = \bigcup U\_{\lambda},\qquad \bigwedge U\_{\lambda} = \mathrm{int}\left(\bigcap U\_{\lambda}\right) $$
